    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card card-widget widget-user ">
                ...
                <div class="widget-user-image admin-custom-table-pos"  style="left:20% !important;">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card custom-card">
                                ...
                                {   ?>
                                        <tr class="custom-table-tr">
                                            <td><?=$mainData[$i]["OrderId"]?></td>
                                            <td><?=$mainData[$i]["OrderDate"]?></td>
                                            <td><?=$mainData[$i]["CustomerName"]?></td>
                                            <td><?php
                                                $className='';
                                                if(strcmp($mainData[$i]["OrderStageDropDown"],"Cancel")==0)
                                                    $className="badge-danger";
                                                else if(strcmp($mainData[$i]["OrderStageDropDown"],"Processing")==0)
                                                    $className="badge-info";
                                                else if(strcmp($mainData[$i]["OrderStageDropDown"],"Delivered")==0)
                                                    $className="badge-success";
                                                else if(strcmp($mainData[$i]["OrderStageDropDown"],"other")==0)
                                                    $className="badge-warning";
                                                
                                                ?>
                                                <span class="badge <?=$className?>"><?=$mainData[$i]["OrderStageDropDown"]?></span>
                                            </td>
                                            <td><?=$mainData[$i]["OrderTotal"]?></td>
                                            <td class="custom-table-td">
                                                <a id="<?=$mainData[$i]["OrderId"]?>" class="updatedata" data-toggle="modal" data-target="#insert-modal" ><i class="fa fa-edit admin-custom-color-opp" ></i></a>
                                                <a id="<?=$mainData[$i]["OrderId"]?>" class="deletedata"><i class="fa fa-trash admin-custom-color-opp" ></i></a>
                                                <a href="<?=site_url('Admin/BillPrintDifferent/'.$mainData[$i]["OrderId"])?>" ><i class="fa fa-print admin-custom-color-opp" ></i></a>
                                                <a href="<?=site_url('Admin/OrderPrintDifferent/'.$mainData[$i]["OrderId"])?>" ><i class="fa fa-file admin-custom-color-opp" ></i></a>
